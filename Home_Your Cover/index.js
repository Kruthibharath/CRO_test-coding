      ...
      ***************************/
      ...
      '</div>'; 
        ...
        $('#buildings').click(function(e){
            document.cookie ="typeOfCover = buildings;path=/;expires=0";
            buildingsSelection();
            e.preventDefault();
        });
      ...
      var ncd_tooltips = '<dd id="container-ncd">'+
        ...
        function contentsSelection(){
            $('div.seg-label-large.cover_selected').removeClass('cover_selected');
            $('span.cover_title.selected_title').removeClass('selected_title');
            $('#contents').find('.cover_title').addClass('selected_title');
            $('#contents').addClass('cover_selected');
        ...
        $('#contents').click(function(e){
            document.cookie = "typeOfCover = contents;path=/;expires=0";
            contentsSelection();
            e.preventDefault();
        });
       ...
       /******************* Buildings and Content Cover Section*****************/
        ...
        function buildingsContentsSelection(){
            $('div.seg-label-large.cover_selected').removeClass('cover_selected');
            $('#buildings_contents').addClass('cover_selected');
            $('span.cover_title.selected_title').removeClass('selected_title');
            $('#buildings_contents').find('.cover_title').addClass('selected_title');
        ...
        $('#buildings_contents').click(function(e){
            document.cookie = "typeOfCover = bothBuildingsContents;path=/;expires=0";
            buildingsContentsSelection();
            e.preventDefault();
        });
        
        /**********************Check for type of cover selected*******************/
        function coverTypeSelectionCookie(){
           if($('#next').data('clicked')){
            return;
          }
          setTimeout(function(){
            if (document.cookie.indexOf("typeOfCover=buildings") >= 0 ) {
                buildingsSelection();  
                console.log("Buildings is selected!!!");
              }
              if (document.cookie.indexOf("typeOfCover=contents") >= 0 ) {
                contentsSelection();
              }
              if (document.cookie.indexOf("typeOfCover=bothBuildingsContents") >= 0 ) {
                buildingsContentsSelection();
                console.log("Buildings and Contents is selected!!!");
              }
          },400);
        }
      
       /************************* onClick of "Next" button, page reloads and hides the warning message & default behaviour. 
        So delete the existing cookie(typeOfCover) and reset the cookie value again ***************************/
        $('#next').click(function(){
            var selectedCoverType;
            $(this).data('clicked',true);
            /*************** Resetting cookie ************************/ 
            if(document.cookie.indexOf("typeOfCover=buildings") >= 0){
                selectedCoverType = buildings;
                document.cookie = "selectedCover = buildings;path=/;expires=0";
            } else if(document.cookie.indexOf("typeOfCover=contents") >= 0){
                selectedCoverType = contents; 
                document.cookie = "selectedCover = contents;path=/;expires=0";
            } else if(document.cookie.indexOf("typeOfCover=bothBuildingsContents") >= 0){
                selectedCoverType = bothBuildingsContents; 
                document.cookie = "selectedCover = bothBuildingsContents;path=/;expires=0";
            }
            if(!$("#riskAddressSameAsInsuredAddressFlagMessageAnchor").find('label').hasClass('selected')){
                document.cookie = "typeOfCover = ;path=/;expires=Thu, 01 Jan 1970 00:00:00 UTC";
                document.cookie = "addressSelection =  notSelected;path=/;expires=0";
            }else{
                 document.cookie = "addressSelection = ;path=/;expires=0";
                 coverTypeSelection(selectedCoverType);
            }
           
        });
    
        
        function coverTypeSelection(selected){
            ...
            if(selected == "bothBuildingsContents"){
                  document.cookie = "typeOfCover = bothBuildingsContents;path=/;expires=0";
            }  
        }
        
        $("#label_riskAddressSameAsInsuredAddressFlag_0").click(function(){
            if(document.cookie.indexOf("addressSelection=notSelected") >= 0){
                if( $('#buildings').data('clicked') || $('#contents').data('clicked') || $('#buildings_contents').data('clicked') ){
                    return;
                }
                if (document.cookie.indexOf("selectedCover=buildings") >= 0 ) {
                    $('#buildings').trigger('click');  
                }
                if (document.cookie.indexOf("selectedCover=contents") >= 0 ) {
                    $('#contents').trigger('click');  
                }
                if (document.cookie.indexOf("selectedCover=bothBuildingsContents") >= 0 ) {
                    $('#buildings_contents').trigger('click');  
                }
            }  
        });
